...
import {CrosshairMode, createChart, isBusinessDay} from '@/util/lightweight-charts.esm.production.js'
export default {
  name: "TradingView",
  props: {
    item: Object,
  ...
  mounted() {
    this.container = this.$refs.iChart
    this.chartOptions.width = this.$refs.iChart.clientWidth
    this.chartOptions.height = this.$refs.iChart.clientHeight
    this.chart = createChart(this.container, this.chartOptions)
    this.candleSeries = this.chart.addCandlestickSeries(this.candleStickSeriesOptions)
  ...
  watch: {
    item(){
      ...
      this.loadData().then((dataSource)=>{
        this.candleSeries.setData(dataSource.candleStickData)
        this.volumeSeries.setData(dataSource.volumeSeriesData)
  ...
  methods: {
    async loadData(){
      ...
      return new Promise(resolve => {
        setTimeout(() => {
          resolve({candleStickData: vi.candleStickData.slice(0,vi.item.id), volumeSeriesData: vi.volumeSeriesData.slice(0,vi.item.id)});
        }, 1000);
      })
    },
    ...
    handleTooltip(param) {
      ...
      if (!param.time || param.point.x < 0 || param.point.x > this.chartOptions.width || param.point.y < 0 || param.point.y > this.chartOptions.height) {
      ...
      let price = this.candleSeries.series().data().search(this.candleSeries.series().model().crosshairSource().index()).value
      let volume = param.seriesPrices.get(this.volumeSeries)
      this.toolTip.innerHTML = '<div style="font-size: 13px;"><b>O</b>: ' + Math.round(price[0] * 100) / 100 + '<br/><b>H</b>: ' + Math.round(price[1] * 100) / 100 + '<br/><b>L</b>: ' + Math.round(price[2] * 100) / 100 + '<br/><b>C</b>: ' + Math.round(price[3] * 100) / 100 + '<br/><b>Vol</b>: ' + volume + '</div>' +
        '<div><i>' + dateStr + '</i></div>';

      let y = param.point.y;

      let left = param.point.x + toolTipMargin;
      if (left > this.chartOptions.width - toolTipWidth) {
        left = param.point.x - toolTipMargin - toolTipWidth;
      }

      let top = y + toolTipMargin;
      if (top > this.chartOptions.height - toolTipHeight) {
